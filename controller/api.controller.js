...
module.exports.createUser = async (req, res) => {
    ...
    if (!username.trim().length || !password.trim().length || !repassword.trim().length) {
    ...
    if (!userCheck) {
        ...
        await user.save();
        return res.status(201).json({ success: true, message: "Đăng kí tài khoản thành công" });
    }
...
module.exports.getAllPost = async (_, res) => {
    try {
